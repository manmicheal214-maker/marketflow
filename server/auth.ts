...
export async function verifyPassword(password:string,encoded:string) {
  ...
  try { const key=await scrypt(password,salt,64) as Buffer; const expected=Buffer.from(hash,"hex"); return expected.length===key.length&&timingSafeEqual(expected,key); } catch { return false; }
}
function parseCookies(header?:string) {
  if(!header)return {};
  return Object.fromEntries(header.split(";").map(p=>{const i=p.indexOf("=");return i<0?["",""]:[p.slice(0,i).trim(),decodeURIComponent(p.slice(i+1).trim())];}).filter(([k])=>k));
}
function setCookie(res:Response,token:string) {
  const secure=process.env.NODE_ENV==="production"?"; Secure":"";
  res.setHeader("Set-Cookie",`${COOKIE}=${encodeURIComponent(token)}; Max-Age=${SESSION_DAYS*86400}; Path=/; HttpOnly; SameSite=Lax${secure}`);
}
...
async function createSession(userId:number) {
  ...
  sqlite.prepare("INSERT INTO sessions (token_hash,user_id,expires_at) VALUES (?,?,?)").run(await hashToken(token),userId,new Date(Date.now()+SESSION_DAYS*86400000).toISOString());
  return token;
}
async function getUserBySession(token:string|undefined) {
...
export function registerAuthRoutes(app:Express) {
  ...
  app.post("/api/auth/register",async(req,res,next)=>{try{
    ...
    const result=sqlite.prepare("INSERT INTO users (username,email,password_hash,full_name) VALUES (?,?,?,?)").run(uname,normalized,await hashPassword(password),fullName.trim().slice(0,120));
  ...
  app.post("/api/auth/login",async(req,res,next)=>{try{
    ...
    if(normalized==="demo@marketflow.io"&&password==="demo"&&(!user||!user.passwordHash)){
      ...
      if(user&&!user.passwordHash){sqlite.prepare("UPDATE users SET password_hash=? WHERE id=?").run(await hashPassword(password),user.id);user.passwordHash=await hashPassword(password);}
    }
