...
export function registerAuthRoutes(app:Express){
 ...
 app.post("/api/auth/register",async(req,res,next)=>{try{const {email,password,fullName,username}=req.body??{};if(typeof email!=="string"||typeof password!=="string"||typeof fullName!=="string")return res.status(400).json({message:"Email, password, and full name are required"});if(password.length<8)return res.status(400).json({message:"Password must be at least 8 characters"});const normalized=email.trim().toLowerCase();if(!/^\S+@\S+\.\S+$/.test(normalized))return res.status(400).json({message:"Invalid email address"});if(sqlite.prepare("SELECT id FROM users WHERE email=?").get(normalized))return res.status(409).json({message:"An account with that email already exists"});const base=(typeof username==="string"&&username.trim())||normalized.split("@")[0];const uname=base.toLowerCase().replace(/[^a-z0-9_-]/g,"-").slice(0,40)||`user-${Date.now()}`;if(sqlite.prepare("SELECT id FROM users WHERE username=?").get(uname))return res.status(409).json({message:"That username is already taken"});const result=sqlite.prepare("INSERT INTO users (username,email,password_hash,full_name) VALUES (?,?,?,?)").run(uname,normalized,await hashPassword(password),fullName.trim().slice(0,120));const user=sqlite.prepare("SELECT id,username,email,password_hash AS passwordHash,full_name AS fullName,avatar_color AS avatarColor,created_at AS createdAt FROM users WHERE id=?").get(result.lastInsertRowid) as AuthUser;setCookie(res,await createSession(user.id));const {passwordHash:_,...safe}=user;res.status(201).json(safe);}catch(e){next(e);}});
 app.post("/api/auth/login",async(req,res,next)=>{try{const {email,password}=req.body??{};if(typeof email!=="string"||typeof password!=="string")return res.status(400).json({message:"Email and password are required"});const user=sqlite.prepare("SELECT id,username,email,password_hash AS passwordHash,full_name AS fullName,avatar_color AS avatarColor,created_at AS createdAt FROM users WHERE email=?").get(email.trim().toLowerCase()) as AuthUser|undefined;if(!user||!user.passwordHash||!(await verifyPassword(password,user.passwordHash)))return res.status(401).json({message:"Invalid email or password"});setCookie(res,await createSession(user.id));const {passwordHash:_,...safe}=user;res.json(safe);}catch(e){next(e);}});
